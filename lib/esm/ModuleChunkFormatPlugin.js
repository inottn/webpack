...
const { generateModuleExecCode } = require("../javascript/StartupHelpers");
const {
	chunkHasJs,
	getCompilationHooks,
...
/** @typedef {import("../javascript/StartupHelpers").ModuleIds} ModuleIds */
						...
						if (hotUpdateChunk) {
							...
							if (entries.length > 0) {
								...
								if (currentModuleIds.length > 0) {
									startupSource.add(
										`${generateModuleExecCode(currentModuleIds, true)}\n`
									);
								}
