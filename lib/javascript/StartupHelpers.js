 ...
 */
const generateModuleExecCode = (moduleIds, final) =>
	`${final ? EXPORT_PREFIX : ""}(${moduleIds.map(runModule).join(", ")});`;

module.exports.generateModuleExecCode = generateModuleExecCode;
...
module.exports.generateEntryStartup = (
	...
	const outputCombination = (chunks, moduleIds, final) => {
		if (chunks.size === 0) {
			runtime.push(generateModuleExecCode(moduleIds, final));
		} else {
